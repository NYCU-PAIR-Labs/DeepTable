"""Extend combined_path_vocab.json with TabFact v2 first-col headers + values.

Mirrors extend_combined_vocab_fetaqa_leftpath_variant.py. TabFact v2 introduces
...
vocab from TabFact v1). first_col_value entries are mostly new leaf strings.

Preserves all existing IDs. Appends new TabFact-v2-only entries at the end.
"""
...
def main():
    ...
    train_ctr, train_roles = collect_all_path_nodes(
        DATA / "tabfact_tpe_v2_train.json", also_track_role=True)
    test_ctr, test_roles = collect_all_path_nodes(
        DATA / "tabfact_tpe_v2_test.json", also_track_role=True)
    print(f"\nTabFact v2 train:")
    ...
    print(f"\nNew entries added by TabFact v2: {len(new_tokens):,}")
    print(f"Combined vocab: {len(existing)} → {len(final_vocab)}")

    print("\nTabFact v2 test OOV vs FINAL combined vocab:")
    for role in ("top_path", "left_parent", "left_leaf"):
    ...
    meta["tabfact_v2_new_entries"] = len(new_tokens)
