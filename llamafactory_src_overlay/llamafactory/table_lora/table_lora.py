...
# Module-level registry for paper's special tokens.
# The list MUST be ordered to match the PromptEncoder virtual token indices.
# i.e. SPECIAL_TOKEN_IDS[0] = id of [TAB], [1] = [ROW], [2] = [CELL].
# Must be set before the first forward pass that uses TableLoRA's prompt
# encoder branch (typically right after model loading in tuner.py).
SPECIAL_TOKEN_IDS: List[int] = []


def set_special_token_ids(ids: List[int]) -> None:
    """Register the token ids that PeftModel_new.forward should route through
    the prompt encoder instead of the base model word embeddings."""
    global SPECIAL_TOKEN_IDS, _PROMPT_ENCODER_CALL_COUNT
    SPECIAL_TOKEN_IDS = list(ids)
    _PROMPT_ENCODER_CALL_COUNT = 0
    print(f"[table_lora] Registered special token ids -> prompt encoder local indices: "
          f"{ {tid: i for i, tid in enumerate(SPECIAL_TOKEN_IDS)} }")


# Per-process counter so we can prove the prompt encoder is actually invoked
# during smoke tests / real training. The counter is reset every time
# set_special_token_ids() is called.
_PROMPT_ENCODER_CALL_COUNT: int = 0
    ...
    **Attributes**:
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        folders = ["."]
        # Only include adapter subfolders (skip checkpoint-*, predict, runs, etc.)
        for entry in entries:
            if not os.path.isdir(os.path.join(model_id, entry)):
                continue
            if entry.startswith("checkpoint-") or entry in ("predict", "runs"):
                continue
            if not os.path.exists(os.path.join(model_id, entry, "adapter_config.json")):
                continue
...
class PeftModelForCausalLM_new(PeftModel):
    ...
    def _table_lora_make_inputs_embeds(self, input_ids):
        """Build the prompt-encoder-aware input embeddings.

        Used by both training (`forward`) and inference (`prepare_inputs_for_generation`).
        Returns inputs_embeds where:
          - normal token positions go through self.word_embeddings as usual
          - special token positions ([TAB], [ROW], [CELL]) are replaced by the prompt encoder output

        SPECIAL_TOKEN_IDS is the module-level registry populated at train startup;
        its order MUST match the prompt encoder virtual token indices.
        """
        global SPECIAL_TOKEN_IDS, _PROMPT_ENCODER_CALL_COUNT
        if not SPECIAL_TOKEN_IDS:
            raise RuntimeError(
                "TableLoRA prompt encoder branch was triggered but SPECIAL_TOKEN_IDS is empty. "
                "Call set_special_token_ids([...]) after loading the tokenizer (in tuner.py)."
            )

        inputs_embeds = torch.zeros(
            (input_ids.shape[0], input_ids.shape[1], self.word_embeddings.embedding_dim),
            device=input_ids.device,
        )
        special_ids_tensor = torch.tensor(SPECIAL_TOKEN_IDS, device=input_ids.device, dtype=input_ids.dtype)

        # mask=True  -> normal token, route through word_embeddings
        # mask=False -> special token, route through prompt encoder
        is_special = torch.isin(input_ids, special_ids_tensor)
        mask = ~is_special

        word_embeds = self.word_embeddings(input_ids[mask])

        # Map each special token id to its prompt-encoder local index (0..N-1).
        special_input_ids = input_ids[is_special]
        local_ids = torch.zeros_like(special_input_ids)
        for local_idx, sid in enumerate(SPECIAL_TOKEN_IDS):
            local_ids = torch.where(
                special_input_ids == sid,
                torch.full_like(local_ids, local_idx),
                local_ids,
            )
        ...
        if local_ids.numel() == 0:
            # No special tokens in this batch (e.g. decoding-time single-token forward).
            # Skip the prompt encoder call entirely; otherwise embedding(empty) is fine
            # but we also skip the diagnostics counter so it stays meaningful.
            pass
        else:
            if peft_config.inference_mode:
                prompt_tuning_emb = prompt_encoder.embedding(local_ids)
            else:
                prompt_tuning_emb = prompt_encoder(local_ids)

            # One-shot diagnostics so smoke tests can verify the encoder is invoked.
            _PROMPT_ENCODER_CALL_COUNT += 1
            if _PROMPT_ENCODER_CALL_COUNT in (1, 50, 200):
                _uniq = sorted(set(local_ids.flatten().tolist()))
                print(
                    f"[table_lora] PROMPT_ENCODER_CALL #{_PROMPT_ENCODER_CALL_COUNT} "
                    f"local_idx_uniq={_uniq} num_special_tokens={local_ids.numel()}",
                    flush=True,
                )

            prompt_tuning_emb = prompt_tuning_emb.to(word_embeds.dtype)
            inputs_embeds = inputs_embeds.to(word_embeds.dtype)
            inputs_embeds[is_special] = prompt_tuning_emb
    ...
    def _table_lora_set_row_col_ids_on_modules(self, row_ids, col_ids):
        """Push the current row_ids/col_ids tensors onto every Linear module so the 2D LoRA
        forward pass can read them. row_ids and col_ids must have shape compatible with
        the input_ids that will be fed in the same forward call."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            # Special Token Encoder (paper §3.1) — see _table_lora_make_inputs_embeds for details.
            inputs_embeds = self._table_lora_make_inputs_embeds(input_ids)

            # 2D LoRA: push current row/col ids to every Linear so its forward can read them.
            self.row_ids = row_ids
            self.col_ids = col_ids
            self._table_lora_set_row_col_ids_on_modules(row_ids, col_ids)
    ...
    def prepare_inputs_for_generation(self, *args, task_ids: Optional[torch.Tensor] = None, **kwargs):
        ...
        if peft_config.is_prompt_learning:
            ...
            #         model_kwargs["input_ids"] = None
                ...
                # === TableLoRA inference path ===
                # We must replicate exactly what training-time `forward()` does:
                #   1. Build inputs_embeds via the prompt encoder split (same helper).
                #   2. Push the current row_ids/col_ids onto the Linear modules so 2D LoRA works.
                #
                # Two important fixes vs the original repo code:
                #   (a) The mask logic must use the explicit special-token-id set, not
                #       `input_ids < vocab_size` (broken on DeepSeek where the new tokens
                #       fall inside vocab range).
                #   (b) self.row_ids must NOT be reset to None each step. It is set once
                #       by `generate()` from the dataset-provided row_ids/col_ids; on
                #       subsequent decoding steps we shrink it to the per-step shape.
                ...
                if self.row_ids is None:
                    # No row/col ids were provided to generate(). Fall back to all-zeros
                    # (2D LoRA effectively disabled). Print a warning so we notice.
                    print(
                        "[table_lora] WARNING: prepare_inputs_for_generation got self.row_ids=None. "
                        "Generate kwargs likely missed row_ids/col_ids. Falling back to zeros.",
                        flush=True,
                    )
                    cur_row_ids = torch.zeros((bs, cur_seq_len), dtype=torch.long, device=input_ids.device)
                    cur_col_ids = torch.zeros((bs, cur_seq_len), dtype=torch.long, device=input_ids.device)
                else:
                    full_row_ids = self.row_ids
                    full_col_ids = self.col_ids
                    # Make sure they're on the right device and are tensors
                    if not torch.is_tensor(full_row_ids):
                        full_row_ids = torch.as_tensor(full_row_ids, dtype=torch.long)
                    if not torch.is_tensor(full_col_ids):
                        full_col_ids = torch.as_tensor(full_col_ids, dtype=torch.long)
                    full_row_ids = full_row_ids.to(input_ids.device)
                    full_col_ids = full_col_ids.to(input_ids.device)

                    if cur_seq_len == full_row_ids.shape[-1]:
                        # Prefill: input_ids is the full prompt, use the full row/col ids
                        cur_row_ids = full_row_ids
                        cur_col_ids = full_col_ids
                    elif cur_seq_len == 1:
                        # Decoding: just produced one new token; new tokens are response,
                        # not in the table → row=0, col=0.
                        cur_row_ids = torch.zeros((bs, 1), dtype=torch.long, device=input_ids.device)
                        cur_col_ids = torch.zeros((bs, 1), dtype=torch.long, device=input_ids.device)
                    else:
                        # Some intermediate slice (rare). Take the trailing slice or pad with 0.
                        if cur_seq_len <= full_row_ids.shape[-1]:
                            cur_row_ids = full_row_ids[:, -cur_seq_len:]
                            cur_col_ids = full_col_ids[:, -cur_seq_len:]
                        else:
                            pad_len = cur_seq_len - full_row_ids.shape[-1]
                            zero_pad_r = torch.zeros((bs, pad_len), dtype=torch.long, device=input_ids.device)
                            zero_pad_c = torch.zeros((bs, pad_len), dtype=torch.long, device=input_ids.device)
                            cur_row_ids = torch.cat([full_row_ids, zero_pad_r], dim=-1)
                            cur_col_ids = torch.cat([full_col_ids, zero_pad_c], dim=-1)

                self._table_lora_set_row_col_ids_on_modules(cur_row_ids, cur_col_ids)

                # Build inputs_embeds via the same path forward() uses (so prompt encoder
                # is exercised exactly the same way during inference as during training).
                inputs_embeds = self._table_lora_make_inputs_embeds(input_ids)
    ...
    def generate(self, *args, **kwargs):
        ...
        try:
            if not peft_config.is_prompt_learning:
                ...
                # Pop row_ids/col_ids if present (the dataset / collator passes them
                # via the trainer's generation_inputs dict). They are NOT valid kwargs
                # for the base model's generate(), so we remove them from kwargs and
                # cache them on self for prepare_inputs_for_generation to consume.
                self.row_ids = kwargs.pop("row_ids", None)
                self.col_ids = kwargs.pop("col_ids", None)
                if self.row_ids is None or self.col_ids is None:
                    print(
                        "[table_lora] WARNING: generate() did not receive row_ids/col_ids in kwargs. "
                        "Inference 2D LoRA will fall back to zeros for the entire prompt.",
                        flush=True,
                    )
...
class DataCollatorForSeq2Seq_new:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        # Stash any extra columns (e.g. row_ids/col_ids) before tokenizer.pad strips them.
        # tokenizer.pad only knows about input_ids/attention_mask/labels/etc., so anything
        # we registered ourselves (the 2D LoRA position ids) gets dropped silently otherwise.
        TOKENIZER_KEYS = {
            "input_ids", "attention_mask", "labels", "token_type_ids",
            "decoder_input_ids", "decoder_attention_mask", "special_tokens_mask",
            "position_ids",
        }
        extra_keys = [k for k in features[0].keys() if k not in TOKENIZER_KEYS]
        extras = {k: [feature[k] for feature in features] for k in extra_keys}
        ...
        # Re-attach the extras as proper batched tensors. They were already padded to
        # equal length earlier in this function (see the loop above), so we can stack.
        for k, vals in extras.items():
            arr = np.array([
                v if isinstance(v, np.ndarray) else np.asarray(v, dtype=np.int64)
                for v in vals
            ])
            if return_tensors == "pt":
                features[k] = torch.tensor(arr, dtype=torch.long)
            else:
                features[k] = arr
...
def load_table_lora():
    ...
    # Helper methods used by both forward() and prepare_inputs_for_generation()
    PeftModelForCausalLM._table_lora_make_inputs_embeds = PeftModelForCausalLM_new._table_lora_make_inputs_embeds
    PeftModelForCausalLM._table_lora_set_row_col_ids_on_modules = PeftModelForCausalLM_new._table_lora_set_row_col_ids_on_modules
