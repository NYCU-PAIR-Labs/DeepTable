...
import torch
from peft import LoraConfig, LoraModel, PeftModel, TaskType, get_peft_model, PromptEncoderConfig
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        # TableLoRA: never merge_and_unload, because TableLoRA's prompt encoder and 2D LoRA
        # both rely on the PeftModel wrapper's dynamic forward / prepare_inputs_for_generation.
        # Merging would silently drop the prompt encoder branch entirely.
        if os.environ.get("TABLE_LORA_ENABLED") == "1":
            is_mergeable = False
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        if model_args.use_unsloth:
            ...
            # TableLoRA: add P-tuning encoder config for special token encoder
            # Paper uses P-tuning (word embedding + linear/MLP encoder), not Prompt Tuning
            if os.environ.get("TABLE_LORA_ENABLED") == "1":
                from llamafactory.table_lora.table_lora import get_peft_model_table_lora
                pt_config = PromptEncoderConfig(
                    task_type=TaskType.CAUSAL_LM,
                    num_virtual_tokens=3,  # <TABLE>, <ROW>, <COL>
                )
                model = get_peft_model_table_lora(model, [pt_config, lora_config])
            else:
