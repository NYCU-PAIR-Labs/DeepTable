...
class DataArguments:
    ...
    emb_lora: bool = field(
        default=False,
        metadata={"help": "Whether to enable TableLoRA (2D LoRA with special token encoder for table data)."},
    )
    use_sab: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether to enable SAB (Structural Attention Bias) on top of TableLoRA. "
                "Adds a per-(layer, head) learnable row/col attention bias initialised to "
                "zero. Requires emb_lora=true (needs row_ids/col_ids in the batch). "
                "Both the eager and SDPA attention paths are patched; only "
                "flash_attn='fa2' is unsupported and is downgraded to 'auto'."
            )
        },
    )
