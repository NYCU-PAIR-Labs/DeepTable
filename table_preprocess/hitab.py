...
# --- DeepTable patch: raw HiTab files are located via deeptable_paths rather than
# the hard-coded `src/table_preprocess/hitab/` of the original TableLoRA release,
# and are loaded lazily so that importing this module (which table_preprocess.py
# does unconditionally) does not require the HiTab files to be present.
import sys as _sys
from pathlib import Path as _Path
_sys.path.insert(0, str(_Path(__file__).resolve().parents[1]))
from deeptable_paths import HITAB_DIR, require_dir, require_file  # noqa: E402

_HITAB_HINT = ("Download the raw HiTab files with `bash setup.sh`, or set "
               "DEEPTABLE_HITAB_DIR to the directory holding them.")

question2table_id = {}


def _load_question2table_id():
    """Build the question+answer -> table_id map from HiTab's train_samples.jsonl."""
    if question2table_id:
        return question2table_id
    path = require_file(HITAB_DIR / "train_samples.jsonl", _HITAB_HINT)
    with open(path) as f:
...
def preprocess_table(dataset, tokenizer, prompt_tuning=False, max_length=1024, save_original_table=False, pretraining=False):
    hmt_dir = require_dir(HITAB_DIR / "hmt", _HITAB_HINT)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if "table_id" not in dataset[i]:
            table_id = _load_question2table_id()[question + ", " + answer]
        else:
...
def preprocess_hitab(model_name="meta-llama/Llama-2-7b-chat-hf", max_length=4000, prompt_tuning=False, save_original_table=False, pretraining=False):
    ...
    #                    }))
    with open(require_file(HITAB_DIR / "hitab_test.json", _HITAB_HINT)) as f:
